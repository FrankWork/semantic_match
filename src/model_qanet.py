...
def dot_product_attention(q,
    ...
    with tf.variable_scope(scope, default_name="dot_product_attention", reuse = reuse):
        ...
        if mask is not None:
            ...
            mask = tf.expand_dims(tf.expand_dims(mask, axis=1), axis=1) # [batch, 1, 1, memory_length]
...
def optimized_trilinear_for_attention(args, c_maxlen, q_maxlen, input_keep_prob=1.0,
    ...
    with tf.variable_scope(scope):
        ...
        subres0 = tf.tile(dot(droped_args[0], weights4arg0), 
                          tf.convert_to_tensor([1, 1, q_maxlen]))
        subres1 = tf.tile(tf.transpose(dot(droped_args[1], weights4arg1), perm=(0, 2, 1)), 
                          tf.convert_to_tensor([1, c_maxlen, 1]))
...
class ModelQANet(object):
  def __init__(self, params, word2vec, features, labels, training=False):
    ...
    c = highway(s1, size=embed_dim, scope = "highway", dropout=dropout, reuse=None)
    q = highway(s2, size=embed_dim, scope = "highway", dropout=dropout, reuse=True)

    c_mask = tf.sequence_mask(len1, dtype=tf.float32)
    q_mask = tf.sequence_mask(len2, dtype=tf.float32)

    # Encoding
    c = residual_block(c, num_blocks = 1, num_conv_layers = 4, kernel_size = 7,
                mask = c_mask, num_filters = hidden_size, num_heads = num_heads,
                seq_len = len1, scope = "Encoder",
                bias = False, dropout = dropout)
    q = residual_block(q, num_blocks = 1, num_conv_layers = 4, kernel_size = 7,
                mask = q_mask, num_filters = hidden_size, num_heads = num_heads, 
                seq_len = len2, scope = "Encoder",
